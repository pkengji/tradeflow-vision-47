...
// Optional: kleine Visualisierung (siehe unten MiniRange.tsx)
import MiniRange from '@/components/app/MiniRange.tsx';
...
export default function TradeDetailPanel({ positionId }: Props) {
  ...
  // --- Mutations (identisch zu deinem TradeDetail)
        ...
        <CardContent className="space-y-3">
          {!posLoading && position && (
            <>
              ...
              {/* Mini-Grafik (Entry/SL/TP/Mark) falls Daten vorhanden */}
              <div className="pt-2">
                <MiniRange
                  entry={position.entry_price ?? null}
                  sl={position.sl ?? null}
                  tp={position.tp ?? null}
                  mark={position.mark_price ?? null}
                />
              </div>
            </>
          )}
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="flex gap-2">
                  <Button variant={action === 'sltp' ? 'default' : 'outline'} onClick={() => setAction('sltp')}>SL / TP setzen</Button>
                  <Button variant={action === 'close' ? 'default' : 'outline'} onClick={() => setAction('close')}>Position schließen</Button>
                </div>
                ...
                {action === 'sltp' && (
                  <div className="grid gap-3">
                    <div className="grid gap-1.5">
                      ...
                      <Input id="sl" placeholder="z.B. 2.4512" inputMode="decimal" value={sl} onChange={(e) => setSl(e.target.value)} />
                    </div>
                    <div className="grid gap-1.5">
                      <Label htmlFor="tp">TP-Preis (optional)</Label>
                      <Input id="tp" placeholder="z.B. 2.4890" inputMode="decimal" value={tp} onChange={(e) => setTp(e.target.value)} />
                    </div>
                  </div>
                )}
