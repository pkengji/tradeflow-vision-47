...
export default function EquityChart({
  ...
  return <div className="relative touch-none">
      <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} style={{
      height: 'auto',
      maxHeight: isMobile ? '350px' : '300px',
      minHeight: isMobile ? '280px' : 'auto'
    }} preserveAspectRatio="none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onClick={handleDoubleTap} onMouseMove={e => {
      ...
      if (dataIndex >= 0 && dataIndex < validData.length) {
    ...
    }} onMouseLeave={() => setHoveredPoint(null)} className="w-full mx-0 px-[200px] py-[200px]">
        {/* Background */}
        ...
        const yPos = y(value);
        return <line key={`grid-${i}`} x1={padLeft} y1={yPos} x2={width - pad} y2={yPos} stroke="hsl(var(--border))" strokeWidth={1} strokeDasharray="4,4" />;
      })}
        
        {/* Y-axis labels */}
        {yTickValues.map((value, i) => {
        const yPos = y(value);
        return <text key={`label-${i}`} x={padLeft - 8} y={yPos} textAnchor="end" alignmentBaseline="middle" className="fill-muted-foreground" style={{
          fontSize: '11px'
        }}>
        ...
        const xPos = x(xs[idx]);
        return <text key={idx} x={xPos} y={height - padBottom + 20} textAnchor="middle" className="fill-muted-foreground" style={{
          fontSize: '11px'
        }}>
        ...
        <g clipPath="url(#chart-clip)">
          <path d={path} fill="none" stroke="currentColor" strokeWidth={2} transform={`translate(${panX}, 0) scale(${zoom}, 1)`} style={{
          transformOrigin: `${padLeft}px center`
        }} />
        </g>
        
        {hoveredPoint && !isNaN(hoveredPoint.x) && !isNaN(hoveredPoint.y) && <circle cx={hoveredPoint.x} cy={hoveredPoint.y} r={4} fill="currentColor" />}
      </svg>
      
      {hoveredPoint && <div className="absolute bg-background border rounded-lg shadow-lg p-3 pointer-events-none z-10 whitespace-nowrap" style={{
      left: (() => {
      ...
      transform: (() => {
        ...
        return 'translateX(-50%)'; // Tooltip zentriert
          ...
          <div className={`text-sm font-medium ${hoveredPoint.point.pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
