...
export default function BotDetail() {
  ...
  const [marginUnit, setMarginUnit] = useState<"percent_bot" | "percent_trade" | "usdt_bot" | "usdt_trade">(
    "percent_bot",
  );
  ...
  const togglePairSelection = (symbol: string) => {
    setSelectedNewPairs((prev) => (prev.includes(symbol) ? prev.filter((s) => s !== symbol) : [...prev, symbol]));
  };

  const addPairs = () => {
    if (selectedNewPairs.length === 0) return;

    const newPairs: BotPair[] = selectedNewPairs
        ...
        <Card>
          ...
          <CardContent className="space-y-4 pt-6">
            <div>
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Bot Name" className="mt-1" />
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div
                className={`flex items-center gap-2 ${marginType === "auto" ? "opacity-50 pointer-events-none" : ""}`}
              >
                <select
                  value={marginUnit}
              ...
              <DialogContent className="sm:max-w-md">
                          ...
                          return (
                            <CommandItem
                              ...
                              className={`cursor-pointer ${
                                selectedNewPairs.includes(s.symbol) ? "bg-primary/20 border border-primary" : ""
                              }`}
                            >
                              {s.icon ? (
                                <img
                                ...
                                <div className="mr-2 h-5 w-5 rounded-full bg-muted flex items-center justify-center">
                                  <span className="text-[10px] font-medium">
                                    {s.base_currency?.slice(0, 2) || s.symbol.slice(0, 2)}
                                  </span>
                                </div>
                              )}
                              <span className="font-medium">{s.symbol}</span>
                              <span className="ml-2 text-xs text-muted-foreground">
                                {s.base_currency || s.symbol.replace("USDT", "")}
                              </span>
                            </CommandItem>
                          );
                        })}
                ...
                <div className="flex justify-end gap-2 mt-4">
                  ...
                  <Button onClick={addPairs} disabled={selectedNewPairs.length === 0}>
                    {selectedNewPairs.length > 0 ? `${selectedNewPairs.length} Pair(s) hinzufügen` : "Hinzufügen"}
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="divide-y divide-border">
                    ...
                    <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 overflow-hidden">
                      {info?.icon ? (
                        ...
                        <div className="w-6 h-6 rounded-full bg-muted flex items-center justify-center">
                          <span className="text-[10px] font-medium">
                            {info?.base_currency?.slice(0, 2) || pair.symbol.slice(0, 2)}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              {filteredPairs.length === 0 && (
                <div className="text-sm text-muted-foreground text-center py-8">Keine Pairs gefunden.</div>
              )}
            </div>
          </CardContent>
        </Card>
